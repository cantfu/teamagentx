...
} from '../core/agent/room-env-vars.js';
...
const chatRoomSchema = {
  ...
  properties: {
    ...
    collapsedAt: { type: 'string', nullable: true },
...
export async function chatRoomGateway(app: FastifyInstance) {
  ...
  app.put<{ Params: ChatRoomParams; Body: UpdateChatRoomBody }>('/chatrooms/:id', {
      ...
      response: {
        200: {
          ...
          properties: {
            success: { type: 'boolean' },
            data: chatRoomSchema,
            skippedReservedKeys: { type: 'array', items: { type: 'string' } },
          },
        },
        404: {
          type: 'object',
    ...
    // 规整环境变量：剔除非法 key，拆出命中保留键的部分（只持久化可用键，保留键反馈给前端）
    let skippedReservedKeys: string[] = [];
    if (data.envVars !== undefined) {
      if (data.envVars === null || data.envVars.trim() === '') {
        data.envVars = null;
      } else {
        const parsed = parseRoomEnvVars(data.envVars);
        const { accepted, skippedKeys } = partitionRoomEnvVars(parsed);
        skippedReservedKeys = skippedKeys;
        data.envVars = accepted.length > 0 ? JSON.stringify(accepted) : null;
      }
    }
  ...
  // Collapse chatroom
  app.patch<{ Params: ChatRoomParams }>('/chatrooms/:id/collapse', {
    schema: {
      description: '折叠群聊',
      tags: ['ChatRooms'],
      params: {
    ...
    try {
      const chatRoom = await chatRoomService.collapse(id);
      return reply.send({ success: true, data: chatRoom });
    } catch (error: any) {
      if (error.code === 'P2025') {
        return reply.code(404).send({ success: false, error: '群聊不存在' });
  ...
  // Uncollapse chatroom
  app.patch<{ Params: ChatRoomParams }>('/chatrooms/:id/uncollapse', {
    schema: {
      description: '取消折叠群聊',
      tags: ['ChatRooms'],
      params: {
    ...
    try {
      const chatRoom = await chatRoomService.uncollapse(id);
      return reply.send({ success: true, data: chatRoom });
    } catch (error: any) {
      if (error.code === 'P2025') {
        return reply.code(404).send({ success: false, error: '群聊不存在' });
